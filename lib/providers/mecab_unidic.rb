  ...
  class Provider
    class MecabUnidic < Ve::Provider
      ...
      def parse(text, options = {})
        ...
        Ve::Parse::MecabUnidic.new(text, output)
      end
      ...
      def initialize(text, output)
        ...
        output.each_with_index do |line, index|
          ...
          if line =~ %r{^ EOS $}x
            ...
            # The parsed token
            token[:type] = :parsed
            token[:literal] = md["surface"]
            token[:hatsuon] = md["pron"]
            token[:reading] = md["lForm"]
            token[:inflection_type] = md["cType"]
            token[:inflection_form] = md["cForm"]
            token[:lemma], token[:lemma_source] = md["lemma"].split("-")
            parts_of_speech = md["pos"].split('-')
            [:pos, :pos2, :pos3, :pos4].each_with_index do |attr, i|
              token[attr] = parts_of_speech[i]
            end
        ...
        begin
          while token = tokens.next
            ...
            when MEISHI
              ...
              case token[:pos3]
              when FUKUSHIKANOU#, SAHENKANOU, KEIYOUDOUSHIGOKAN, NAIKEIYOUSHIGOKAN
                if tokens.more?
            ...
            when JODOUSHI
              pp "JODOUSHI"
              pos = Ve::PartOfSpeech::Postposition # TODO: Should this be Auxiliary?

              if token[:inflection_type] == JODOUSHI_TA
                attach_to_previous = true
              end

              # if [TE, DE, BA].include?(token[:literal])
              #   attach_to_previous = true
              # end
...
Ve::Manager.register(Ve::Provider::MecabUnidic, :ja)
